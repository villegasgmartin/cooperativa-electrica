...
// Estilo de modales:
    ...
    const totalPendientesMes = reservasDelMesVisible.filter(
        (r) => r.estadoCalendario === 'pendiente'
    ).length;

    const totalCompletadasMes = reservasDelMesVisible.filter(
    ...
    return (
        <Box sx={{ width: '90%', margin: 'auto', marginTop: '30px', marginBottom: 6 }}>
            <GlobalStyles
                ...
                />
        <Typography variant="h5" sx={{ fontFamily: 'InterTight', fontWeight: 800, mb: 1 }}>
            Calendario de reservas
        </Typography>

        <Typography variant="subtitle1" gutterBottom sx={{ fontFamily: 'InterTight', fontWeight: 700 }}>
            Mostrando {reservasDelMesVisible.length} reservas del mes visible
        </Typography>
        ...
        <Dialog open={openDayModal} onClose={handleCloseDay} fullWidth maxWidth="md">
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
                <Box>
                    ...
                    {selectedDay
                        ? selectedDay.format('dddd DD [de] MMMM [de] YYYY').charAt(0).toUpperCase() +
                        selectedDay.format('dddd DD [de] MMMM [de] YYYY').slice(1)
                        : ''}
                    </Typography>
    ...
    <DialogContent dividers>
        ...
        {/* Botón cobertura + tipo de inmueble en una misma línea */}
        <Grid item xs={12}>
            <Box
            sx={{
                display: 'flex',
